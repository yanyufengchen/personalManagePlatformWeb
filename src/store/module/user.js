...
import { setToken, getToken, formatRoutes, getMenuByRouter} from '@/libs/util'
...
export default {
  ...
  mutations: {
    ...
    setUserRoutes (state, userRoutes) {
      ...
      // setUserSessionRoutes(userRoutes)
    },
  ...
  actions: {
    ...
    // 查询用户权限菜单
    getUserMenus ({ commit }, { vm }) {
      return new Promise((resolve, reject) => {
        getUserMenus()
          .then(res => {
            if (res.data.success) {
              const userMenus = res.data.data
              const oRoutes = formatRoutes(userMenus)
              commit('setUserRoutes', oRoutes)
              // vm.$router.addRoutes(oRoutes)
              const menus = getMenuByRouter(oRoutes, vm.$store.state.user.access)
